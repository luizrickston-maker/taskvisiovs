    ...
    'team', (
      SELECT json_build_object(
        ...
        'members', (
          ...
          FROM workspace_members WHERE user_id = p_user_id AND is_active = true
        )
      )
      FROM workspace_members WHERE user_id = p_user_id AND is_active = true
    ),
    
    -- Financeiro: Custos Corporativos (Investimentos/Gastos)
    'investments', (
      SELECT json_build_object(
        ...
        'items', (
          ...
          FROM (SELECT * FROM corporate_costs WHERE user_id = p_user_id LIMIT 20) cc
        )
      )
      FROM corporate_costs WHERE user_id = p_user_id
    ),
