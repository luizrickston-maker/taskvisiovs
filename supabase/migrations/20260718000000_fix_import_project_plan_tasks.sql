-- =====================================================
-- FIX: import_project_plan() — tasks não eram inseridas
--
-- Bug: o `RETURNING ... INTO v_stage_record` do INSERT em
-- project_stages sobrescrevia a variável do FOR LOOP, perdendo
-- o array `tasks` original. O loop interno recebia NULL e
-- criava 0 tarefas sem disparar erro.
--
-- Correção: guardar o array `tasks` em uma variável separada
-- (`v_stage_tasks`) ANTES do INSERT, e usar essa variável no
-- loop interno das tasks.
-- =====================================================
...
DECLARE
  ...
  v_stage_tasks jsonb;  -- FIX: guarda tasks antes do INSERT que sobrescreve v_stage_record
BEGIN
  ...
  LOOP
    ...
    -- FIX: guarda tasks ANTES do INSERT em project_stages,
    -- porque o RETURNING abaixo sobrescreve v_stage_record.
    ...
    v_task_order := 0;
    -- FIX: usa v_stage_tasks (que tem o array de tasks original),
    -- em vez de v_stage_record->'tasks' (que agora é o registro
    -- recém-inserido de project_stages, sem tasks).
